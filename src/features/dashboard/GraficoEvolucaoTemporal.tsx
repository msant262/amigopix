...
export const GraficoEvolucaoTemporal: React.FC<GraficoEvolucaoTemporalProps> = ({ 
  kpis, 
  timeRange 
}) => {
  ...
  const generateTimeSeriesData = () => {
    ...
    switch (timeRange) {
      ...
      case 'all':
        ...
        const oldestEmprestimo = emprestimos.reduce((oldest, emp) => 
          emp.dataInicio < oldest ? emp.dataInicio : oldest, emprestimos[0]?.dataInicio || now
        );
    ...
    for (let date = new Date(startDate); date <= now; date.setDate(date.getDate() + interval)) {
      ...
      const emprestimosAteData = emprestimos.filter(emp => emp.dataInicio <= currentDate);
      const pagamentosAteData = pagamentos.filter(pag => pag.data <= currentDate);
      
      // Calcular valores acumulados até esta data
      const totalEmprestados = emprestimosAteData.reduce((sum, emp) => sum + emp.principal, 0);
      const totalRecebido = pagamentosAteData.reduce((sum, pag) => sum + pag.valor, 0);
      const totalJurosAcumulados = emprestimosAteData.reduce((sum, emp) => sum + emp.jurosAcumulado, 0);
      ...
      const emprestimosAtivos = emprestimosAteData.filter(emp => emp.status === 'ativo').length;
      
      data.push({
        data: formatDate(currentDate, 'dd/MM'),
